...
int main() {
    ...
    int navioDiagonal1[3] = {3, 3, 3}; // Declaração e inicialização do Vetor de tipo inteiro para implementar o 1º Navio em posição Diagonal Principal, que ocupará 3 posições em uma diagonal do Tabuleiro.
    int navioDiagonal2[3] = {3, 3, 3}; // Declaração e inicialização do Vetor de tipo inteiro para implementar o 2º Navio em posição Diagonal Secundária, que ocupará 3 posições em uma diagonal do Tabuleiro.
    ...
    for (int i = 0; i < 3; i++) { // Loop for para posicionar o Navio Horizontal nos índices [8][5], [8][6] e [8][7], ocupando 3 posições na horizontal a partir da coordenada 9 x F do Tabuleiro.
        tabuleiro[8][5 + i] = navioHorizontal[i];
    }

    for (int i = 0; i < 4; i++) { // Loop for para posicionar o Navio Vertical nos índices [6][2], [7][2], [8][2] e [9][2], ocupando 4 posições na vertical a partir da coordenada C x 7 do Tabuleiro.
        tabuleiro[6 + i][2] = navioVertical[i];
    }

    for (int i = 0; i < 3; i++) { // Loop for para posicionar o 1º Navio Diagonal nos índices [1][2], [2][3] e [3][4], ocupando 3 posições na diagonal a partir da coordenada 2 x C do Tabuleiro.
        tabuleiro[1 + i][2 + i] = navioDiagonal1[i];
    }

    for (int i = 0; i < 3; i++){ // Loop for para posicionar o 2º Navio Diagonal(Secundária) nos índices [3][8], [4][7] e [5][6], ocupando 3 posições na diagonal inversa a partir da coordenada 4 x I do Tabuleiro.
